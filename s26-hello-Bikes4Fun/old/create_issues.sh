#!/bin/bash
# Create GitHub issues for SE-3200 Semester Plan
# Run from se3200_web_app directory

# Issue 1: Demo Data Generator
gh issue create \
  --title "Project 2: Demo Data Generator (Jan 28)" \
  --body "## Feature
Demo Data Generator - creates random appointments for TV app testing

## Integration
Generates test data matching Dementia TV's \`calendar_events\` schema
...
- [ ] Ajax — fetch placeholder image API (e.g., \`picsum.photos\`)
...
# Issue 2: Medication Dose Log
gh issue create \
  --title "Project 3: Medication Dose Log (Feb 23)" \
  --body "## Feature
Medication Dose Log — track when medications are taken
...
## Files
\`medications.html\`, \`medications.js\`, \`meds.json\`, \`server/app.py\`, \`server/doses.json\`" \
  --label "enhancement"

# Issue 3: Calendar Events CRUD
gh issue create \
  --title "Project 4: Calendar Events CRUD (Mar 25)" \
  --body "## Resource
Calendar Events

## Integration
Shares database with Dementia TV app (\`dementia_tv.db\`)
...
## Files
\`server/app.py\`, \`server/database.py\`, \`calendar.html\`, \`calendar.js\`, \`README.md\`" \
  --label "enhancement"

# Issue 4: Deployment
gh issue create \
  --title "Deployment (Apr 8)" \
  --body "## Integration
Configure shared database path for TV connection
...
gh issue create \
  --title "Project 5: Authentication (Apr 22)" \
  --body "## Feature
User accounts + protected calendar
...
## Files
\`server/app.py\`, \`server/database.py\`, \`login.html\`, \`register.html\`, \`auth.js\`" \
  --label "enhancement"
